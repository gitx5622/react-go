...
func ForgotPassword(c *gin.Context) {
	...
	err = Config.DB.Debug().Model(entities.User{}).Where("email = ?", user.Email).Take(&user).Error
	if err != nil {
	...
	//generate the token:
	token := utils.TokenHash(user.Email)
	resetPassword.Email = user.Email
	resetPassword.Token = token

	resetDetails, err := resetPassword.SaveDatails(Config.DB)
	if err != nil {
	...
	response, err := providers.SendMail.SendResetPassword(resetDetails.Email, os.Getenv("SENDGRID_FROM"), resetDetails.Token, os.Getenv("SENDGRID_API_KEY"), os.Getenv("APP_ENV"))
	if err != nil {
		c.JSON(http.StatusUnprocessableEntity, gin.H{
			"status": http.StatusUnprocessableEntity,
			"error":  err,
...
func ResetPassword(c *gin.Context) {
	...
	err = Config.DB.Debug().Model(entities.ResetPassword{}).Where("token = ?", requestBody["token"]).Take(&resetPassword).Error
	if err != nil {
	...
	if requestBody["new_password"] != "" && requestBody["retype_password"] != "" {
		...
		//update the password
		user.Prepare()
		err := user.UpdatePassword(Config.DB)
		if err != nil {
		...
		_, err = resetPassword.DeleteDatails(Config.DB)
		if err != nil {
